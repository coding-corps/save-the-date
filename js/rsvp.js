$(document).ready(function () {
  $("#submit_rsvp").click(function () {
    ...
    $(
      "#rsvp_form .form-group input[required], #rsvp_form .form-group textarea[required]"
    ).each(function () {
      ...
      $.post(
        ...
        function (response) {
          if (response.type == "error") {
            ...
            output =
              '<div class="alert alert-danger">' + response.text + "</div><br>";
          } else {
            output = '<h6 class="text-center">' + response.text + "</h6>";
            //reset values in all input fields
            $(
              "#rsvp_form  input[required=true], #rsvp_form textarea[required=true]"
            ).val("");
            $("#rsvp_form .form-group").slideUp(); //hide form after success
            $("html, body").animate(
              { scrollTop: $("#rsvp_form .form-group").offset().top - 150 },
              2000
            );
          }
